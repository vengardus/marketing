        ...
        <div className="flex items-center   my-10 ">
          <div className="lg:flex min-w-0 lg:flex-1 items-center">
            ...
            <div className="min-w-0 flex-1 px-8 p-4 ">
              <p id={`title` + post.id} className="  leading-10 text-3xl pb-4 font-semibold transition duration-300 ease-in-out">{post.title.length > 80 ? post.title.slice(0, 79) : post.title}</p>
              <div className="">
                ...
                <p className="mt-4 text-lg font-regular text-gray-800 leading-8">{post.description.length > 150 ? post.description.slice(0, 149) : post.description}</p>
              </div>
            </div>
          </div>
        </div>
